...
<div class="pt-userdetails">
	<?= $lang['details']['yourplan'] ?>
	<span class="badge <?= ($u_plan == '1' ? 'bg-gy' : ($u_plan == '2' ? 'bg-gr' : ($u_plan == '3' ? 'bg-v' : 'bg-o'))) ?>">
		<i class="fas fa-trophy"></i> <?= ($u_plan ? db_get("plans", "plan", $u_plan) : $lang['details']['freeplan']) ?>
	</span>
	...
	<form class="pt-senduserdetails">
		...
		<div id="thumbnails"><img src="<?= ($u_photo ? path . '/' . $u_photo : nophoto) ?>" onerror="this.src='<?= nophoto ?>'" class="nophoto" /></div>
		...
		<div class="form-row mb-2">
			<div class="col">
				<div class="pt-form-i pt-countries">
					<span class="pt-icon"><i class="flag-icon flag-icon-<?= ($u_country ? strtolower($u_country) : 'us') ?>"></i></span>
