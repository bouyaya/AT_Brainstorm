	...
	?>
		<style>
			...
			#example-async .actions ul li a[href="#finish"] {
				<?php if (isset($survey_colors['button_border_width'])) : ?>border-width: <?= $survey_colors['button_border_width'] ?>px;
				<?php endif; ?><?php if (isset($survey_colors['button_border_style'])) : ?>border-style: <?= $survey_colors['button_border_style'] ?>;
				<?php endif; ?><?php if (isset($survey_colors['button_border1_color'])) : ?>border-color: <?= $survey_colors['button_border1_color'] ?>;
				<?php endif; ?><?php if (!$survey_colors['bg_gradient'] && $survey_colors['bg1_color1']) : ?>background: linear-gradient(to right, <?= $survey_colors['bg1_color1'] ?> 0%, <?= $survey_colors['bg1_color2'] ?> 80%, <?= $survey_colors['bg1_color2'] ?> 100%) !important;
				<?php endif; ?><?php if (isset($survey_colors['bg_gradient']) && isset($survey_colors['bg1_color1'])) : ?>background: <?= $survey_colors['bg1_color1'] ?> !important;
				<?php endif; ?><?php if (isset($survey_colors['txt_color1'])) : ?>color: <?= $survey_colors['txt_color1'] ?>;
				<?php endif; ?>
			}
			...
			#example-async .actions ul li a[href="#next"] {
				<?php if (isset($survey_colors['button_border_width'])) : ?>border-width: <?= $survey_colors['button_border_width'] ?>px;
				<?php endif; ?><?php if (isset($survey_colors['button_border_style'])) : ?>border-style: <?= $survey_colors['button_border_style'] ?>;
				<?php endif; ?><?php if (isset($survey_colors['button_border2_color'])) : ?>border-color: <?= $survey_colors['button_border2_color'] ?>;
				<?php endif; ?><?php if (!$survey_colors['bg_gradient'] && $survey_colors['bg2_color1']) : ?>background: linear-gradient(to right, <?= $survey_colors['bg2_color1'] ?> 0%, <?= $survey_colors['bg2_color2'] ?> 80%, <?= $survey_colors['bg2_color2'] ?> 100%) !important;
				<?php endif; ?><?php if (isset($survey_colors['bg_gradient']) && isset($survey_colors['bg2_color1'])) : ?>background: <?= $survey_colors['bg2_color1'] ?> !important;
				<?php endif; ?><?php if (isset($survey_colors['txt_color2'])) : ?>color: <?= $survey_colors['txt_color2'] ?>;
				<?php endif; ?>
			}

			.pt-likertscale .rating-group label,
			.pt-survey.pt-newsurvey .choice+label {
				<?php if (isset($survey_colors['label_bg'])) : ?>background: <?= $survey_colors['label_bg'] ?>;
				<?php endif; ?>
			}

			.pt-likertscale .rating-group {
				<?php if (isset($survey_colors['label_bg'])) : ?>border-color: <?= $survey_colors['survey_bg'] ?>;
				<?php endif; ?>
			}

			.step-link:before {
				<?php if (!$rs['bg_gradient'] && $rs['bg_color1']) : ?>background: linear-gradient(to right, <?= $rs['bg_color1'] ?> 0%, <?= $rs['bg_color2'] ?> 80%, <?= $rs['bg_color2'] ?> 100%);
				<?php endif; ?><?php if ($rs['bg_gradient'] && $rs['bg_color1']) : ?>background: <?= $rs['bg_color1'] ?>;
				<?php endif; ?>
			}

			<?php if ($request == "su" && $survey_colors['survey_bg']) : ?>body.pt-nouser {
				background: <?= $survey_colors['survey_bg'] ?>
			}

			<?php endif; ?><?php if (isset($survey_colors['survey_bg'])) : ?>.pt-surveypage .pt-body .pt-dots a.active,
			...
			<?php endif; ?><?php if (isset($survey_colors['input_bg'])) : ?>input[type=text],
			input[type=password],
			input[type=phone],
			input[type=email],
			input[type=number],
