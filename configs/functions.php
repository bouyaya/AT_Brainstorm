...
function fh_title()
{
	...
	switch ($pg) {
		case 'survey':
			$title = db_get("survies", "title", $id) . ' - ' . site_title;
			break;
		case 'plans':
			$title = 'Plans - ' . site_title;
			break;
		case 'dashboard':
			$title = 'Dashboard - ' . site_title;
			break;
		case 'rapport':
			$title = 'Rapport - ' . site_title;
			break;
		case 'responses':
			$title = 'Responses - ' . site_title;
			break;
		case 'newsurvey':
			$title = 'New Survey - ' . site_title;
			break;
		case 'userdetails':
...
function fh_ago($tm = '', $at = true, $rcs = 0)
{
	...
	if ($tm > $pr_month) {
		...
		$no = floor($no);
		if ($no <> 1 && $lang['rtl'] != 'rtl_true') $pds[$v] .= ($lang['lang'] == 'en' ? 's' : '');
