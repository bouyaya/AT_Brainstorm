...
function fh_pagination($query, $per_page = 10, $url = '?', $page = 1, $rr = false)
{
	global $db, $page;
	$sql = $db->query("SELECT COUNT(id) as `num` FROM " . prefix . "{$query}");
	$row = $sql->fetch_array();
	$total = $rr ? db_rows($query, "MAX(id)") : $row['num'];
	$adjacents = "1";
